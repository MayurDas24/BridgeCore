...
**[`deploy.yml`](.github/workflows/deploy.yml)** — manual trigger only, since it needs a
real AWS account. Given credentials, it:

1. Re-runs tests (last cheap place to stop a bad build).
2. Assumes an AWS role via **OIDC** — no long-lived AWS keys exist in GitHub. A stolen
...
3. Builds and pushes, tagged by commit SHA — never only `latest`, or "roll back to the
   previous build" has no meaning.
4. Deploys and waits for service stability.
5. Smoke-tests `/health` and verifies the *deployed* version matches the commit.
6. On any failure, rolls back to the task definition captured *before* the update.
